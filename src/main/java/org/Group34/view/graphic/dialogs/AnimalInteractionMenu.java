...
public class AnimalInteractionMenu implements Disposable {
    ...
    private static final String[] OPTIONS = {"Information", "Pet", "Shear", "Feed", "Sell", "Move"};
    private int selectedOption = 0;

    // Icon display system
    public enum IconType {
        HEART, WOOL, CARROT, COINS, FOOTSTEPS
    }
    ...
    private void activateOption() {
        ...
        // Don't close menu immediately for actions that show dialogs
        if (OPTIONS[selectedOption].equals("Information") ||
                (OPTIONS[selectedOption].equals("Shear") && animal.getAnimalType() != AnimalType.SHEEP) ||
                (OPTIONS[selectedOption].equals("Shear") && animal.getAnimalType() == AnimalType.SHEEP && (!animal.isFed() || !animal.isCollected())) ||
                (OPTIONS[selectedOption].equals("Feed") && player.getAmountOfItem(Vegetable.CARROT) <= 0)) {
            // Keep menu open for these cases
            continueRendering = false;
        } else {
            // Close menu for other actions but continue rendering icons
            active = false;
    ...
    private void shearAnimal() {
        if (animal.getAnimalType() == AnimalType.SHEEP) {
            // Check if sheep is ready for shearing (has been fed and enough days passed)
            if (animal.isFed() && animal.isCollected()) {
                // Collect wool product
                org.Group34.model.enums.animals.Product woolProduct = animal.collectProduct();
                if (woolProduct != null) {
                    // Show wool icon
                    activeIcon = IconType.WOOL;
                    iconTimer = ICON_DURATION;
                } else {
                    // Show information dialog for this case since it's an error condition
                    com.badlogic.gdx.scenes.scene2d.ui.Dialog shearDialog = new com.badlogic.gdx.scenes.scene2d.ui.Dialog("", skin);
                    shearDialog.text("This sheep is not ready for shearing yet!");
                    shearDialog.button("OK");
                    shearDialog.show(stage);
                }
            } else {
                // Show information dialog for this case since it's an error condition
                com.badlogic.gdx.scenes.scene2d.ui.Dialog shearDialog = new com.badlogic.gdx.scenes.scene2d.ui.Dialog("", skin);
                shearDialog.text("This sheep needs to be fed and have enough time before shearing!");
                shearDialog.button("OK");
                shearDialog.show(stage);
            }
        } else {
            // Show information dialog for this case since it's an error condition
            com.badlogic.gdx.scenes.scene2d.ui.Dialog shearDialog = new com.badlogic.gdx.scenes.scene2d.ui.Dialog("", skin);
            shearDialog.text("Only sheep can be sheared!");
            shearDialog.button("OK");
            shearDialog.show(stage);
        }
    }
    ...
    private void moveAnimal() {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (newX >= 0 && newX < currentSpace.width() &&
                ...
                // Move the animal
                currentSpace.placingEntity(currentX, currentY, null); // Clear old position
                animal.setX(newX);
                animal.setY(newY);
